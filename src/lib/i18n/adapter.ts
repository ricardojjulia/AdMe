...
export async function getActiveCatalog(locale: string): Promise<Record<string, string>> {
  ...
  // Attempt REST fetch using NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY (defaulting to production project jmtwkiizirfussizdjmh)
  ...
  if (supabaseUrl && supabaseAnonKey) {
    try {
      ...
      console.error('REST catalog fetch failed:', err);
    }
  }

  // Fallback to pg direct connection if REST fails or is not available (e.g. in local CLI/tests)
  try {
    const p = getPool();
