...
ALTER TABLE localization_locales ENABLE ROW LEVEL SECURITY;
ALTER TABLE localization_catalog_versions ENABLE ROW LEVEL SECURITY;
ALTER TABLE localization_validation_reports ENABLE ROW LEVEL SECURITY;
ALTER TABLE localization_review_assignments ENABLE ROW LEVEL SECURITY;
ALTER TABLE localization_review_decisions ENABLE ROW LEVEL SECURITY;
ALTER TABLE localization_activation_history ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist to support idempotency
DROP POLICY IF EXISTS "Allow select for everyone" ON localization_locales;
DROP POLICY IF EXISTS "Allow select for everyone" ON localization_catalog_versions;
DROP POLICY IF EXISTS "Allow select for everyone" ON localization_validation_reports;
DROP POLICY IF EXISTS "Allow select for everyone" ON localization_review_assignments;
DROP POLICY IF EXISTS "Allow select for everyone" ON localization_review_decisions;
DROP POLICY IF EXISTS "Allow select for everyone" ON localization_activation_history;

-- RLS Read Policies (public/auth read only)
CREATE POLICY "Allow select for everyone" ON localization_locales FOR SELECT USING (true);
CREATE POLICY "Allow select for everyone" ON localization_catalog_versions FOR SELECT USING (true);
CREATE POLICY "Allow select for everyone" ON localization_validation_reports FOR SELECT USING (true);
CREATE POLICY "Allow select for everyone" ON localization_review_assignments FOR SELECT USING (true);
CREATE POLICY "Allow select for everyone" ON localization_review_decisions FOR SELECT USING (true);
CREATE POLICY "Allow select for everyone" ON localization_activation_history FOR SELECT USING (true);
